...
(function() {
  ...
  function validateStep(index) {
    ...
    if (index === 0) {
      ...
      // Allow 'Cliente Generico' as valid
      if (clientName === 'Cliente Generico') return true;
    }
    if (index === 1) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (clientInput) {
        clientInput.addEventListener('input', function() {
            if (clientInput.value !== 'Cliente Generico') {
...
// PATCH: Allow 'Cliente Generico' to pass validation and skip client_id lookup
function validateStep(index) {
    if (index === 0) {
        const clientInput = document.getElementById('client-list');
        const clientName = clientInput.value.trim();
        if (clientInput.dataset.generalClient === 'true' || clientName === 'Cliente Generico') {
            return true;
        }
        if (!clientName) {
            showNotification('Please enter/select a client name', 'error');
...
// PATCH: In collectInvoiceData, treat 'Cliente Generico' as a special case for required fields
...
InvoiceForm.prototype.collectInvoiceData = function() {
    ...
    const clientInput = document.getElementById('client-list');
    if ((clientInput && clientInput.dataset.generalClient === 'true') || (data.client && data.client.name === 'Cliente Generico')) {
        data.client.name = 'Cliente Generico';
        data.client.email = '';
        data.client.address = '';
        data.client.taxId = '';
    }
...
// PATCH: In saveInvoice, skip client_id lookup for 'Cliente Generico'
const ORIGINAL_saveInvoice = InvoiceForm.prototype.saveInvoice;
InvoiceForm.prototype.saveInvoice = async function() {
    const invoiceData = this.collectInvoiceData();
    const clientInput = document.getElementById('client-list');
    if ((clientInput && clientInput.dataset.generalClient === 'true') || (invoiceData.client && invoiceData.client.name === 'Cliente Generico')) {
...
// Add highlight style for Cliente Generico button
